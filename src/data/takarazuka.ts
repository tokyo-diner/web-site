...
export const takarazuka = (): Message => ({
  title: `出来立てパンを、その場で。`,
  lead: `
コンセントマーケット to table 宝塚店では、「日々の食卓を彩るパンと食べ方」をコンセプトにしたランチをお楽しみいただけます（予約制）。パンのおいしさを引き出す料理として「季節の煮込みプレート」と「季節のポタージュプレート」をご用意し、焼き立てのパンと一緒に提供しています。
...
もちろん、本店と同じようにテイクアウトもご利用いただけます。
    `,
  images: GeneratedImages.find(
    (image) => Object.keys(image)[0] === 'takarazukaPaths',
  )!.takarazukaPaths!,
});
